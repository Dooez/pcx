...
: fft_size_(fft_size) {
    ...
    if (fft_size > coherent_size) {
        ...
        ileave_external_ptr_   = &fft_plan::tform_external_ileave<width>;
        ileave_external_r_ptr_ = &fft_plan::rtform_external_ileave<width>;
        return;
    }
...
template<floating_point T, fft_options Opts>
template<uZ Width>
void fft_plan<T, Opts>::tform_external_ileave(T* dst, const T* src) {
    tform_external<Width, 1, 1>(dst, src, forward);
}
template<floating_point T, fft_options Opts>
template<uZ Width, uZ PermWidth, uZ Align>
void fft_plan<T, Opts>::coherent_tform_external_ileave(T* dst, const T* src) {
    coherent_tform_external<Width, PermWidth, 1, 1, Align>(dst, src, forward);
}

template<floating_point T, fft_options Opts>
template<uZ Width, uZ NodeSize>
void fft_plan<T, Opts>::single_load_tform_external_ileave(T* dst, const T* src) {
    single_load_tform_external<Width, NodeSize, 1, 1>(dst, src, forward);
}

template<floating_point T, fft_options Opts>
template<uZ Width>
void fft_plan<T, Opts>::rtform_external_ileave(T* dst, const T* src) {
    tform_external<Width, 1, 1>(dst, src, reverse);
}
template<floating_point T, fft_options Opts>
template<uZ Width, uZ PermWidth, uZ Align>
void fft_plan<T, Opts>::coherent_rtform_external_ileave(T* dst, const T* src) {
    coherent_tform_external<Width, PermWidth, 1, 1, Align>(dst, src, reverse);
}

template<floating_point T, fft_options Opts>
template<uZ Width, uZ NodeSize>
void fft_plan<T, Opts>::single_load_rtform_external_ileave(T* dst, const T* src) {
