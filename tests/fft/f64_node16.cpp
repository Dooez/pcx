...
using fX                        = f64;
static constexpr uZ   node_size = 16;
static constexpr auto fX_widths = f64_widths;
template<>
bool test_fft<fX, node_size>(const std::vector<std::complex<fX>>& signal,
                             ...
                             std::vector<fX>&                     twvec) {
    return run_tests<fX, node_size>(fX_widths, low_k, local_tw, half_tw, signal, check, s1, twvec);
};
