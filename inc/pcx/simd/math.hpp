...
#include "pcx/simd/x86/traits.hpp"
...
PCX_AINLINE auto add(Lhs lhs, Rhs rhs) {
    ...
    using new_cx_vec = cx_vec<typename vec::value_type, neg_real, neg_imag, Lhs::size(), Lhs::pack_size()>;
...
PCX_AINLINE auto mul_tuples(std::tuple<Lhs...> lhs, std::tuple<Rhs...> rhs) {
    auto res1 = []<uZ... Is> PCX_LBDINLINE(auto lhs, auto rhs, std::index_sequence<Is...>) {
        return std::make_tuple(detail_::mul_step1(std::get<Is>(lhs), std::get<Is>(rhs))...);
    }(lhs, rhs, std::index_sequence_for<Lhs...>{});

    return []<uZ... Is> PCX_LBDINLINE(auto res1, auto lhs, auto rhs, std::index_sequence<Is...>) {
...
template<tight_cx_vec Lhs, tight_cx_vec Rhs>
    requires compatible<Lhs, Rhs>
PCX_AINLINE auto mul(Lhs lhs, Rhs rhs) {
...
PCX_AINLINE auto div_tuples(std::tuple<Lhs...> lhs, std::tuple<Rhs...> rhs) {
    constexpr auto step1 = []<typename L, typename R> PCX_LBDINLINE(L lhs, R rhs) {
        ...
        using new_cx_vec = cx_vec<typename vec::value_type, neg_real, neg_imag, L::size(), L::pack_size()>;
...
template<tight_cx_vec Lhs, tight_cx_vec Rhs>
    requires compatible<Lhs, Rhs>
PCX_AINLINE auto div(Lhs lhs, Rhs rhs) {
