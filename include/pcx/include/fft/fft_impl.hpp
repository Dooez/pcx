#pragma once
#include "pcx/include/fft/btfly_r2.hpp"
...
struct subtransform {
    ...
    static constexpr auto reuse_lowk_tw = val_ce<false>{};
    ...
    PCX_AINLINE static void perform_subtf(cxpack_for<T> auto          dst_pck,
                                          cxpack_for<T> auto          src_pck,
        ...
        if constexpr (!reverse) {
            ...
            while (k_count <= fk) {
                if constexpr (lowk && reuse_lowk_tw)
                    l_tw_data = tw_bak;
    ...
    static void insert_tw_subtf(twiddle_range_for<T> auto& r,
        ...
        if constexpr (align.size_pre() != 1)
            iter_t::insert_tw_iteration(align.size_pre(), r, tw_data, k_count, lowk);

        if (lowk && reuse_lowk_tw) {
            k_count  = 2 * final_k_count / align.size_post() / node_size;
            auto d_k = 2 * k_count / align.size_pre();
            if (k_count > 0) {
                tw_data.start_fft_size *= d_k;
                tw_data.k *= d_k;
                iter_t::insert_tw_iteration(node_size, r, tw_data, k_count, lowk);
            }
        } else {
        ...
        if constexpr (align.size_post() != 1)
            iter_t::insert_tw_iteration(align.size_post(), r, tw_data, k_count, lowk);
    }
};
...
struct sequential_subtransform {
    ...
    static void insert_tw_seq(twiddle_range_for<T> auto& r,
                              any_align auto             align,
                              meta::ce_of<bool> auto     lowk,
                              uZ                         data_size,
                              tw_data_t<T, true>&        tw_data,
    ...
    PCX_AINLINE static auto single_load(cxpack_for<T> auto     dst_pck,
        ...
        //     --tw_data.k;
        auto regroup_tw_fact = [&] PCX_LAINLINE {
        ...
        if constexpr (!reverse) {
            auto btfly_res_0 = btfly_node::forward(data,    //
                                                   lowk,
                                                   btfly_node::next_tw(lowk, reverse, tw_data),
                                                   conj_tw);
            ...
            auto data0    = btfly_node::reverse(data1,    //
                                             lowk,
                                             btfly_node::next_tw(lowk, reverse, tw_data),
                                             conj_tw);
    ...
    static void insert_tw_single_load(twiddle_range_for<T> auto& r,
                                      tw_data_t<T, true>&        tw_data,
                                      meta::ce_of<bool> auto     lowk,
                                      meta::ce_of<bool> auto     half_tw) {
        ...
        using btfly_node = btfly_node_dit<node_size, T, width>;
        auto tws         = btfly_node::make_tw_node(lowk, tw_data.start_fft_size * 2, tw_data.k);
        insert(tws);
        ...
        auto regroup_tw_fact = [&]<uZ TwCount> PCX_LAINLINE(uZ_ce<TwCount>) {
            return [=]<uZ KGroup> PCX_LAINLINE(uZ_ce<KGroup>) {
                ...
                auto k        = tw_data.k * NodeSize * TwCount / 2 + KGroup * TwCount;
...
struct transform {
    ...
    PCX_AINLINE static void perform_tf(cxpack_for<T> auto          dst_pck,
        ...
        const auto batch_align_seq = [=] PCX_LAINLINE {
            constexpr auto      min_align = std::min(dst_pck.value, src_pck.value);
            constexpr auto      pbegin    = log2i(min_align);
            constexpr auto      pend      = log2i(batch_size);
            return []<uZ... Ps> PCX_LAINLINE(uZ_seq<Ps...>) {
                return std::index_sequence<powi(2, pend - 1 - Ps)...>{};
            }(std::make_index_sequence<pend - pbegin>{});
        }();
        constexpr auto bucket_tfsize = uZ_ce<bucket_size / (sequential ? 1 : batch_size)>{};
        ...
        const auto [pass_cnt, rem_k_cnt] = [=] PCX_LAINLINE {
            if constexpr (sequential) {
        ...
        auto tform = [=] PCX_LAINLINE(auto width, auto batch_size, auto dst, auto src, auto tw_data) {
            constexpr auto w_pck = cxpack<width, T>{};

            uZ bucket_cnt     = fft_size / bucket_tfsize;
            uZ bucket_grp_cnt = 1;
            using subtf_t     = subtransform<node_size, T, width>;
            auto subtf        = [&] PCX_LAINLINE(auto  dst_pck,
                                          auto  src_pck,
                                          auto  align,
                                          auto  lowk,
                                          auto  dst,
            ...
            auto coherent_perm = [=] PCX_LAINLINE(auto& permuter, auto pck, auto dst, auto src) {
                permuter.coherent_permute(width, batch_size, reverse, pck, pck, dst, src);
            };

            auto iterate_buckets = [&] PCX_LAINLINE(auto  dst_pck,
                                                    auto  src_pck,
                                                    auto  align,
                                                    auto  k_cnt,
                                                    auto  src,
    ...
    PCX_AINLINE static void perform_tf_rev(cxpack_for<T> auto          dst_pck,
        ...
        auto tform =
            [=] PCX_LAINLINE(auto width, auto batch_size, auto dst, auto src, auto tw_data, auto permuter) {
                constexpr auto w_pck = cxpack<width, T>{};
                using subtf_t        = subtransform<node_size, T, width>;
        ...
        if constexpr (sequential) {
            ...
            auto permute = [&] PCX_LAINLINE(auto width, auto batch_size) {
                auto l_permuter = permuter;
                auto o_src =
    ...
    static void insert_tw_tf(twiddle_range_for<T> auto& r,    //
                             uZ                         fft_size,
                             meta::ce_of<bool> auto     lowk,
                             meta::ce_of<bool> auto     half_tw,
        ...
        if (fft_size <= bucket_tfsize) {
            ...
            [&]<uZ... Is>(uZ_seq<Is...>) {
                auto check_align = [&]<uZ I>(uZ_ce<I>) {
                    ...
                    constexpr auto align = align_param<l_node_size, true>{};
                    if constexpr (sequential)
                        seq_subtf_t::insert_tw_seq(r, align, lowk, fft_size, l_tw_data, half_tw);
                    else
                        subtf_t::insert_tw_subtf(r, align, lowk, fft_size / 2, l_tw_data);
                    return true;
                };
                (void)(check_align(uZ_ce<Is>{}) || ...);
            }(make_uZ_seq<log2i(node_size)>{});
        ...
        auto iterate_buckets  = [&](meta::ce_of<uZ> auto align_node,    //
            ...
            for (uZ i_bg: stdv::iota(0U, bucket_group_count)) {
                ...
                if (i_bg == 0)
                    subtf_t::insert_tw_subtf(r, align, lowk, k_count, l_tw_data);
                else
                    subtf_t::insert_tw_subtf(r, align, not_lowk, k_count, l_tw_data);
            }
        };
        ...
        if (pass_count > 0) {
            const auto k = pre_pass_k_count == 0 ? 1 : pre_pass_k_count;

            const auto final_bucket_group_count = k * 2 * powi(pass_k_count * 2, pass_count - 1);
        ...
        if (sequential) {
            ...
            for (uZ i_bg: stdv::iota(0U, bucket_count)) {
                ...
                if (i_bg == 0)
                    seq_subtf_t::insert_tw_seq(r, seq_align, lowk, bucket_size, l_tw_data, half_tw);
                else
                    seq_subtf_t::insert_tw_seq(r, seq_align, not_lowk, bucket_size, l_tw_data, half_tw);
            }
            return;
        }
    };
